...
The topic-modeling code (`measurement/topic_modeling_lib.py`,
`measurement/topic_modeling.ipynb`,
`measurement/test_topic_modeling_lib.py`) runs in a second,
narrowly-scoped virtualenv on **Python 3.11**, not `norm_env`: `gensim`
has no usable wheel for the Python 3.14 that `norm_env` is pinned to.
Everything else in the project — including the rest of `measurement/` —
keeps using `norm_env`.
...
The last step registers the Jupyter kernel that
`measurement/topic_modeling.ipynb` expects (its saved metadata names the
kernel `topic_modeling_env`). Run the topic-modeling tests with this
venv too:
`topic_modeling_env/bin/python -m pytest measurement/test_topic_modeling_lib.py`.
...
Gold labels for where a new agenda item (Tagesordnungspunkt, TOP) or
thematic unit starts in the presiding officer’s (`pre`) paragraphs, for
two samples of protocols (v1: 47, v2: 48; one row per `pre` paragraph):
...
The annotator reads `labelling/top_boundaries_annotation_input*.csv` and
writes `labelling/top_boundaries_opener_labels*.csv` (tracked in git).
Labelling rules are in
`labelling/top_boundaries_annotation_conventions.md`; the pre-labelling
workflow used for the second half of the annotation (every label
reviewed by hand) is in
`labelling/top_boundaries_prelabelling_method.md`.

## TOP-opener classification

`measurement/top_change/top_change_classification_v1_v2.ipynb`
classifies whether a block of the presiding officer’s paragraphs opens a
new TOP or thematic unit, on both annotation samples (95 protocols).
Consecutive `pre` paragraphs are joined into blocks, also across
interjection-only (`nsc`) gaps. It compares chance baselines, the
rule-based detector (`preprocessing/top_boundaries.py`), bag-of-words
models with nested C/threshold tuning, and a fine-tuned German encoder
(ModernGBERT) with the neighbouring blocks as context.

- Folds are frozen per protocol in
  `measurement/top_change/fold_assignment.csv`, so label fixes don’t
  reshuffle them.
- Out-of-fold predictions are checkpointed (`nested_oof_checkpoint.pkl`,
  `bert_oof_checkpoint.pkl`), so the notebook runs in minutes. The BERT
  folds are trained in a separate process with
  `norm_env/bin/python measurement/top_change/run_bert_cv.py` (about 2.5
  h on an Apple-silicon Mac).
- The held-out test split (1 protocol per state) is not opened yet; the
  final model evaluation is pending.
- `measurement/top_change/top_change_error_analysis.md` records the
  error reviews and the label fixes they led to.

It runs in `norm_env` (`scikit-learn`, `torch` and `transformers` are in
`requirements.txt`). Tests for the rule-based detector run from inside
`preprocessing/`:
